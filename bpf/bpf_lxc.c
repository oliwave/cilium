...
static __always_inline int handle_ipv6_from_lxc(struct __ctx_buff *ctx, __u32 *dst_id)
{
	...
	lb6_ctx_restore_state(ctx, &ct_state_new, &tuple, &proxy_port);
...
skip_service_lookup:
		...
		lb6_ctx_store_state(ctx, &ct_state_new, &tuple, proxy_port);
	}
#endif /* ENABLE_PER_PACKET_LB */
...
skip_service_lookup:
		/* Store state to be picked up on the continuation tail call. */
		lb4_ctx_store_state(ctx, &ct_state_new, &tuple, proxy_port);
	}
#endif /* ENABLE_PER_PACKET_LB */
